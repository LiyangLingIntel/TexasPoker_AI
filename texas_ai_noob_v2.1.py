...
# 下面是一些关于用户信息的变量
MY_NAME = "leonling"  # 玩家名
MY_PLAYER_NAME = "e5f7f0be53aaddb3097efef0f3b32c26"  # 玩家名对应的hashcode
total_bet = 0
my_round_bet = 0
my_chips = 0
player_survived = 0
...
def get_rank(total_cards):
    ...
    # Sraight 顺子
    if rank == 0:
        if value_list[0] + 1 == value_list[1] and value_list[1] + 1 == value_list[2] and value_list[2] + 1 == value_list[3] and value_list[3] + 1 == value_list[4] or value_list[4] == 0xC and value_list[0] + 1 == value_list[1] and value_list[1] + 1 == value_list[2] and value_list[2] + 1 == value_list[3]:
            # Straight Flush 同花顺
...
# version2 修改， 根据官方文档的牌面比较算法做出调整，直接 七选五 得到最大手牌值
def get_hand_value(private, public):
    ...
    # version2 直接用组合的方法 七选五
    cards_index = list(itertools.combinations(range(7), 5))

    for i in range(10):
        total = []
        total.append(my_cards[cards_index[i][0]])
        total.append(my_cards[cards_index[i][1]])
        total.append(my_cards[cards_index[i][2]])
        total.append(my_cards[cards_index[i][3]])
        total.append(my_cards[cards_index[i][4]])
...
def get_hand_strenth(data):
    ...
    remain_cards.sort()
    while remain_cards[-1] == INVALID_INDEX:
        remain_cards.pop(-1)
    ...
    for times in range(PRECISE_VALUE):
        ...
        # 分别给剩下的人分配手牌，并把调用get_hand_value()，计算并他们的牌值放入 hand_value_list中
        ...
        if my_hand_value == hand_value_list[-1]:
            ...
            my_score += (1.00 / (win_player_num - 1.00))
        # print times
    hand_strenth = my_score / float(PRECISE_VALUE)
...
def choose_action_FCR(data):
    ...
    pot_odds = return_rate_list[2]
    # 第一轮发牌，未知公共牌，，以手牌强度作为主要判断依据 return_rate == hand_strenth
    if pot_odds == 1.00:
        if return_rate < 0.01:
            if data["self"]["bet"] == 0:
                ws.send(
...
def update_local_data(action, data):
    ...
    # 新一轮开始的时候， 将本地总奖池金额归零， 然后记录我的本轮投注和持有的筹码数， 以及场上活着的人数
        ...
        round_name = data["table"]["roundName"]
        print round_name
        log_info += "\n\t\t\t  " + round_name + " board:" + str(data["table"]["board"]) + " smale_blind:" + data["smallBlind"] + " bigBlind:" + data["bigBlind"]
        # print my_chips
        for i in data["players"]:
            if i["playerName"] == MY_PLAYER_NAME:
                my_chips = i["chips"]
                my_round_bet = i["roundBet"]
                # log_info += " my_chips:" + str(my_chips) + " my_round_bet:" + str(my_round_bet)
                break
